<template>
  <div class="nejree-teirs mx-3 pt-3 pb-4 select-none">
    ...
    <h4 class="text-center mt-0 mb-2">{{ fullname }}</h4>

    <div class="flex justify-content-center align-items-center text-center">
      <div class="px-4">
        <div class="text-800 text-xs">My Points</div>
        <div class="font-bold text-sm">{{ earnings }}</div>
      </div>
      <div class="px-4">
        <div class="text-800 text-xs">You are in</div>
        <div class="font-bold text-sm">{{ progress }}%</div>
      </div>
    </div>

    <div class="flex flex-wrap justify-content-between py-4 tiers-progress">
      <div
        ...
        <div>
          ...
          <span
            ...
            :class="{
              ...
              '-ml-3': i > items.length - 2 && i <= items.length,
            }"
          >
            {{ tier.name }}
          </span>
      ...
      >
        <div
          class="tier-progress-reached bg-primary-600"
    ...
    <div v-for="tier of items" :key="tier.id">
      <div
        class="bg-white border-round-top-xl overflow-hidden border-none font-medium relative h-7rem"
        :class="{
      ...
      >
        ...
        ></div>
        <div
          class="align-items-center text-xl flex justify-content-start text-white relative z-2 p-3 pb-2"
        >
          <span class="inline-flex mr-2 select-none" v-html="tier.badge"></span>
          <div class="select-none" v-text="tier.name" style="width: 90%"></div>

          <div v-if="tier.name === active_tier" class="w-4 flex align-items-center justify-content-end text-right text-sm">
            <span class="bg-black-alpha-20 py-1 px-2 border-round-lg">Active Tier</span>
          </div>

        </div>

        <div
          class="flex align-items-center justify-content-center text-center relative text-white w-full text-sm border-top-1 border-white-alpha-20 pt-2"
        >
          <div class="px-3 w-6">
